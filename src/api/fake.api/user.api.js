...
//   setTimeout(() => resolve(users), 3500);
// });

// const fetchUserById = (id) => new Promise((resolve) => {
//   setTimeout(() => resolve(users.find(({ _id }) => _id === id)), 1500);
// });
...
const fetchAll = () =>
  new Promise((resolve) => {
    window.setTimeout(() => resolve(JSON.parse(localStorage.getItem('users'))), 2000);
  });
...
const update = (id, data) =>
  new Promise((resolve) => {
    window.setTimeout(() => {
      ...
      const userIndex = usersFromLs.findIndex((u) => u._id === id);
      usersFromLs[userIndex] = { ...usersFromLs[userIndex], ...data };
      localStorage.setItem('users', JSON.stringify(usersFromLs));
      resolve(usersFromLs[userIndex]);
    }, 1500);
  });
